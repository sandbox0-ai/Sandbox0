...
import (
	...
	globalhttp "github.com/sandbox0-ai/sandbox0/global-directory/pkg/http"
...
func main() {
	cfg := config.LoadGlobalDirectoryConfig()
	...
	logger.Info("Starting global-directory", zap.Int("port", cfg.HTTPPort))

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	obsProvider, err := observability.New(observability.Config{
		ServiceName: "global-directory",
		Logger:      logger,
	...
	logger.Info("Global directory shutdown complete")
}
...
func initDatabase(
	ctx context.Context,
	cfg *config.GlobalDirectoryConfig,
	logger *zap.Logger,
...
func runMigrations(ctx context.Context, pool *pgxpool.Pool, cfg *config.GlobalDirectoryConfig, logger *zap.Logger) error {
	schema := cfg.DatabaseSchema
	if schema == "" {
		schema = "gd"
	}
