...
func testMeteringRouteServer(t *testing.T, authMode string) (*Server, *internalauth.Generator, *authn.Issuer) {
	...
	issuer := authn.NewIssuer("internal-gateway", "secret", time.Minute, time.Hour)
	...
	server := &Server{
		router:          gin.New(),
		cfg:             &config.InternalGatewayConfig{AuthMode: authMode},
...
func newTestMeteringObservability(t *testing.T) *observability.Provider {
	...
	provider, err := observability.New(observability.Config{
		ServiceName:    "internal-gateway-metering-route-test",
		Logger:         zap.NewNop(),
		DisableTracing: true,
		DisableMetrics: true,
		DisableLogging: true,
